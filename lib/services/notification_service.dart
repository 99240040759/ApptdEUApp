...
// ─── Background handler — must initialize plugin in background isolate ────────
@pragma('vm:entry-point')
Future<void> firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  // Re-initialize in background isolate (separate from main isolate)
  const initSettings = InitializationSettings(
    android: AndroidInitializationSettings('@drawable/ic_notification'),
  );
  await NotificationService._local.initialize(settings: initSettings);
...
const AndroidNotificationChannel _channel = AndroidNotificationChannel(
  ...
  importance: Importance.high, enableLights: true,
  enableVibration: true, ledColor: Color(0xFFB91C1C),
);
...
const AndroidNotificationDetails _androidDetails = AndroidNotificationDetails(
  ...
  icon: '@drawable/ic_notification', color: Color(0xFFB91C1C),
...
class NotificationService {
  ...
  static Future<void> init() async {
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((msg) {
      onNotificationTap?.call(msg.data['route'] ?? '/');
    });
  ...
  static Future<void> _showLocal({
    ...
    await _local.show(id: id, title: title, body: body,
      notificationDetails: _notifDetails, payload: payload);
  }
}
