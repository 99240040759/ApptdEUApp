  ...
  void _openFile(Circular c) {
    if (c.fileType == 'image') {
      Navigator.push(context, MaterialPageRoute(
        builder: (_) => _ImageViewerPage(url: c.fileUrl, title: c.title)));
    } else {
      _downloadAndOpen(context, c.fileUrl, c.title);
    }
  }
...
// ── Native file opener with download progress ──
...
class _DownloadDialogState extends State<_DownloadDialog> {
  double? _progress; // null = connecting
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: Row(children: [
        ...
        const Text('Opening file', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
      ]),
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 8),
        Text(
          _progress == null ? 'Connecting…' : '${(_progress! * 100).toInt()}%',
          style: const TextStyle(fontSize: 12, color: AppColors.textMuted)),
      ]),
      actions: [
        TextButton(
...
// ── Full-screen image viewer with loading state ──
